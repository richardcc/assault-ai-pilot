...
class EfficientZeroV2BaseNetwork(nn.Module):
    """
    Local base network for EZv2 to avoid direct inheritance from MuZero modules.
    """
    ...
    def __init__(
        ...
        if self.encoder_type == "cnn":
            ...
            self.action_embedding = nn.Embedding(
                self.action_dim, self.hidden_dim * self.observation_height * self.observation_width
            )
            self.dynamics_merge = nn.Sequential(
                nn.Linear(
                    2 * self.hidden_dim * self.observation_height * self.observation_width,
                    self.hidden_dim * self.observation_height * self.observation_width,
                ),
                nn.ReLU(),
            )
    ...
    def recurrent_inference(
        self, hidden: torch.Tensor, action_onehot: torch.Tensor, return_aux: bool = False
    ):
        if self.encoder_type == "cnn":
