...
class _DummyCatalog:
    ...
    def actions(self):
        ...
        target = SimpleNamespace(unit_id="IT_1", alive=True, hp=8, position=HexCoord(3, 0))
        class RangedDirectAttack:
            pass
        attack = RangedDirectAttack()
        attack.action_type = SimpleNamespace(category="ATTACK")
...
def _build_state():
    rl = SimpleNamespace(
        ...
        get_combat_advantage=lambda _t: 0.3,
        get_expected_damage=lambda _t: 1.2,
    )
    en = SimpleNamespace(
        unit_id="IT_1",
        side="IT",
...
def test_lote_e_features_present_and_bounded(monkeypatch):
    ...
    # Tail layout: [..., lote_e(4)]
    assert 0.0 <= float(obs[-4]) <= 1.0
    assert float(obs[-3]) in (0.0, 1.0)
    assert -1.0 <= float(obs[-2]) <= 1.0
    assert -1.0 <= float(obs[-1]) <= 1.0
    assert float(obs[-3]) == pytest.approx(1.0, rel=1e-6, abs=1e-6)
